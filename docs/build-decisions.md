...
Cloudflare is the implementation default so hosting, the form endpoint, storage, and notification email live in one account with no third-party scripts and no added cost. The form is a same-origin Pages Function, which the shipped CSP (`form-action 'self'`, `connect-src 'self'`) already permits without relaxation; an external form provider would require loosening it. This is prepared configuration, not a deployment. The default build disables submission. An owner must create the bindings, choose who receives messages, set HUGO_PARAMS_FORMENABLED=true, and authorize a test before live delivery is accepted.
...
Base URL intentionally defaults to https://example.invalid/. Cloudflare Pages builds derive canonical URLs from CF_PAGES_URL. No domain registration, DNS change, publication, push, staging, or commit is authorized by implementation.
