...
7. Choose hosting and a form provider. This version prepares Netlify Forms; other providers require implementation. Configure recipient and enable collection only with owner approval. Keep the form disabled on generic static hosts.
...
`data/site.yaml` controls the primary UI palette and typeface. Fixed colors inside decorative artwork, the social background, and the dark closing banner are intentional art direction; replace these assets/styles for a fully different design. Font paths are base-path aware; subpath builds are covered by a generated-output test. Netlify form processing on a subpath is not accepted without a live test.

## Maintenance

Hugo and Sass pins live in `.hugo-version` and `.sass-version`; Netlify's versions must match. QA packages are pinned in `package.json`/`package-lock.json`. Update these deliberately and rerun the focused checks. Installers never run during a build, and build helpers never publish.

Official integration references: [Hugo Sass variables](https://gohugo.io/functions/css/sass/), [Hugo text image filter](https://gohugo.io/functions/images/text/), and [Netlify form setup](https://docs.netlify.com/manage/forms/setup/).
