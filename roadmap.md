...
**Current interface step:** A functional low-fidelity project workspace now supports saved briefs, page plans, editable section copy, reordering, human review, and exported design handoffs. Direct AI integration and generation of a client site from that plan remain future work. The existing component catalog is supporting reference at `/site-kit/catalog/`.

**Current Baseline (2026-09-22):**
- **Architecture & Build:** Pinned Hugo Extended 0.166.0 + Dart Sass 1.104.1 via Hugo Pipes (`@use`, css.Sass, `hugo:vars`). System-independent Python build helper (`scripts/build.py`) with preflight output reconciliation protecting untracked, colliding, parent-conflicting, or edited files, plus crash-resilient manifest recording for interrupted publications. Configurable search-engine visibility (`hugo.toml` `params.noindex`, `HUGO_PARAMS_NOINDEX` env var).
- **Hosting & Forms:** Cloudflare Pages default (`wrangler.toml`) with same-origin Pages Function (`functions/api/contact.js`) for durable enquiry storage in Cloudflare KV (`ENQUIRY`), explicit per-deployment intake gating (`ENQUIRY_ENABLED = "true"`), durable storage-first acknowledgment (502 on storage failure; best-effort email notification once stored), optional R2 image bucket declaration (`IMAGES_BUCKET`), strict security headers including HSTS (`static/_headers`), and storage-only public notice pending live verified delivery.
...
- **Scaffolding Engine (`scripts/new_site.py`):** Deterministic creation of independent client copies, stripping the workshop, unused presets, unselected page trees, and unreferenced project images while generating sanitized, unconfigured deployment files (`wrangler.toml`, `docs/cloudflare-setup.md`) with no master KV IDs, bucket names, or active intake variables, and enforcing disabled delivery and `noindex`.
- **Verification Harness:** Deterministic test suites (`scripts/test_factory.py`, `scripts/test_starter.py`, `scripts/test_contact_endpoint.mjs`, `scripts/check_site.py`, `scripts/check_contact.sh`) validating JSON schema, asset integrity, link resolution, output reconciliation (including parent file conflict preflight and crash-resilient manifest writes), subpath deployments, robots visibility, deployment isolation, and contact function failure paths. Playwright/axe browser audit scripts (`scripts/check_workshop.cjs`, `scripts/check_components.cjs`, `scripts/check_workflow.cjs`, `scripts/browser-checks.cjs`) verifying 61 variants, planning workspace persistence/behavior (including 2 MB backup limits), style guide accessibility, and zero overflow.
