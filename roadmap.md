...
- **Scaffolding & Plan Compiler (`scripts/new_site.py`, `scripts/from_plan.py`):** Deterministic creation of independent client copies from any of the 8 presets, with optional palette and font pairing, and a guided mode (`--guided`) with lettered choices. Dedicated compiler (`scripts/from_plan.py`) converting low-fidelity planner JSON exports directly into validated factory presets adhering to module schema contracts, hierarchy rules, and item constraints, with pure in-memory preview validation protecting existing preset files from collision deletion.
