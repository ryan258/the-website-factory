 ...
 try {
  ...
  for(const mode of ['light','dark']){
   ...
   const variants=await page.locator('.kit-variant').count();
    ...
    if(axe.violations.length||variants!==expectedVariants||widths.some(w=>w.overflow))failures.push('expanded '+mode);
  }
  ...
  await page.goto(base+'site-kit/catalog/');await page.screenshot({path:path.join(out,'workshop-desktop.png'),fullPage:true});
